/**
 * @file coachella.c
 * @brief Main file for COACHELLA-LSU library.
 * @author - SCEC 
 * @version 1.0
 *
 * @section DESCRIPTION
...
int coachella_init(const char *dir, const char *label) {
	...
	coachella_configuration = calloc(1, sizeof(coachella_configuration_t));
	coachella_velocity_model = calloc(1, sizeof(coachella_model_t));

	// Configuration file location.
	sprintf(configbuf, "%s/model/%s/data/config", dir, label);

	// Read the configuration file.
	if (coachella_read_configuration(configbuf, coachella_configuration) != SUCCESS)
		tempVal = FAIL;

	// Set up the data directory.
	sprintf(coachella_data_directory, "%s/model/%s/data/%s/", dir, label, coachella_configuration->model_dir);

	// Can we allocate the model, or parts of it, to memory. If so, we do.
	tempVal = coachella_try_reading_model(coachella_velocity_model);
	...
	coachella_total_height_m = sqrt(pow(coachella_configuration->top_left_corner_lat - coachella_configuration->bottom_left_corner_lat, 2.0f) +
						  pow(coachella_configuration->top_left_corner_lon - coachella_configuration->bottom_left_corner_lon, 2.0f));
	coachella_total_width_m  = sqrt(pow(coachella_configuration->top_right_corner_lat - coachella_configuration->top_left_corner_lat, 2.0f) +
						  pow(coachella_configuration->top_right_corner_lon - coachella_configuration->top_left_corner_lon, 2.0f));

	// Let everyone know that we are initialized and ready for business.
	coachella_is_initialized = 1;
 ...
 */
int coachella_query(coachella_point_t *points, coachella_properties_t *data, int numpoints) {
	int i = 0;
	int load_x_coord = 0, load_y_coord = 0, load_z_coord = 0;
	double x_percent = 0, y_percent = 0, z_percent = 0;
	coachella_properties_t surrounding_points[8];
        double lon_e, lat_n;

	int zone = 11;
	int longlat2utm = 0;

        double delta_lon = (coachella_configuration->top_right_corner_lon - coachella_configuration->bottom_left_corner_lon)/(coachella_configuration->nx - 1);
        double delta_lat = (coachella_configuration->top_right_corner_lat - coachella_configuration->bottom_left_corner_lat)/(coachella_configuration->ny - 1);
	...
	for (i = 0; i < numpoints; i++) {
		...
		load_y_coord = (int)(round((lat_n - coachella_configuration->bottom_left_corner_lat) / delta_lat));
		load_x_coord = (int)(round((lon_e - coachella_configuration->bottom_left_corner_lon) / delta_lon));
		load_z_coord = (int)((points[i].depth)/1000);

		// Are we outside the model's X and Y and Z boundaries?
		if (points[i].depth > coachella_configuration->depth || load_x_coord > coachella_configuration->nx -1  || load_y_coord > coachella_configuration->ny -1 || load_x_coord < 0 || load_y_coord < 0 || load_z_coord < 0) {
			data[i].vp = -1;
			data[i].vs = -1;
			data[i].rho = -1;
			data[i].qp = -1;
		...
		x_percent =fmod((lon_e - coachella_configuration->bottom_left_corner_lon), delta_lon) /delta_lon;
		y_percent = fmod((lat_n - coachella_configuration->bottom_left_corner_lat), delta_lat)/
delta_lat;
		z_percent = fmod(points[i].depth, coachella_configuration->depth_interval) / coachella_configuration->depth_interval;

		if (load_z_coord == 0 && z_percent == 0) {
			// We're below the model boundaries. Bilinearly interpolate the bottom plane and use that value.
			load_z_coord = 0;
                   if(coachella_configuration->interpolation) {

			// Get the four properties.
			coachella_read_properties(load_x_coord,     load_y_coord,     load_z_coord,     &(surrounding_points[0]));	// Orgin.
			coachella_read_properties(load_x_coord + 1, load_y_coord,     load_z_coord,     &(surrounding_points[1]));	// Orgin + 1x
			coachella_read_properties(load_x_coord,     load_y_coord + 1, load_z_coord,     &(surrounding_points[2]));	// Orgin + 1y
			coachella_read_properties(load_x_coord + 1, load_y_coord + 1, load_z_coord,     &(surrounding_points[3]));	// Orgin + x + y, forms top plane.

			coachella_bilinear_interpolation(x_percent, y_percent, surrounding_points, &(data[i]));
                  } else {
			coachella_read_properties(load_x_coord,     load_y_coord,     load_z_coord,     &(data[i]));	// Orgin.
                  }
		  ...
		  if( coachella_configuration->interpolation) {
			...
			coachella_read_properties(load_x_coord,     load_y_coord + 1, load_z_coord,     &(surrounding_points[2]));	// Orgin + 1y
			coachella_read_properties(load_x_coord + 1, load_y_coord + 1, load_z_coord,     &(surrounding_points[3]));	// Orgin + x + y, forms top plane.
			coachella_read_properties(load_x_coord,     load_y_coord,     load_z_coord - 1, &(surrounding_points[4]));	// Bottom plane origin
			coachella_read_properties(load_x_coord + 1, load_y_coord,     load_z_coord - 1, &(surrounding_points[5]));	// +1x
			coachella_read_properties(load_x_coord,     load_y_coord + 1, load_z_coord - 1, &(surrounding_points[6]));	// +1y
			coachella_read_properties(load_x_coord + 1, load_y_coord + 1, load_z_coord - 1, &(surrounding_points[7]));	// +x +y, forms bottom plane.

			coachella_trilinear_interpolation(x_percent, y_percent, z_percent, surrounding_points, &(data[i]));
                    } else {
...
void coachella_read_properties(int x, int y, int z, coachella_properties_t *data) {
	...
	int location = z * (coachella_configuration->nx * coachella_configuration->ny) + (y * coachella_configuration->nx) + x;

	// Check our loaded components of the model.
	if (coachella_velocity_model->vp_status == 2) {
		// Read from memory.
		ptr = (float *)coachella_velocity_model->vp;
		data->vp = ptr[location];
	} else if (coachella_velocity_model->vp_status == 1) {
		// Read from file.
 ...
 */
void coachella_trilinear_interpolation(double x_percent, double y_percent, double z_percent,
							 coachella_properties_t *eight_points, coachella_properties_t *ret_properties) {
	coachella_properties_t *temp_array = calloc(2, sizeof(coachella_properties_t));
	coachella_properties_t *four_points = eight_points;

	coachella_bilinear_interpolation(x_percent, y_percent, four_points, &temp_array[0]);

	// Now advance the pointer four "coachella_properties_t" spaces.
	four_points += 4;

	// Another interpolation.
	coachella_bilinear_interpolation(x_percent, y_percent, four_points, &temp_array[1]);

	// Now linearly interpolate between the two.
	coachella_linear_interpolation(z_percent, &temp_array[0], &temp_array[1], ret_properties);
...
void coachella_bilinear_interpolation(double x_percent, double y_percent, coachella_properties_t *four_points, coachella_properties_t *ret_properties) {
	...
	coachella_linear_interpolation(x_percent, &four_points[0], &four_points[1], &temp_array[0]);
	coachella_linear_interpolation(x_percent, &four_points[2], &four_points[3], &temp_array[1]);
	coachella_linear_interpolation(y_percent, &temp_array[0], &temp_array[1], ret_properties);
...
{
  int verlen;
  verlen = strlen(coachella_version_string);
  if (verlen > len - 1) {
    verlen = len - 1;
  }
  memset(ver, 0, len);
  strncpy(ver, coachella_version_string, verlen);
  return 0;
}
...
void print_error(char *err) {
	fprintf(stderr, "An error has occurred while executing COACHELLA. The error was:\n\n");
	fprintf(stderr, "%s", err);
	fprintf(stderr, "\n\nPlease contact software@scec.org and describe both the error and a bit\n");
	fprintf(stderr, "about the computer you are running COACHELLA on (Linux, Mac, etc.).\n");
}
...
int coachella_try_reading_model(coachella_model_t *model) {
	double base_malloc = coachella_configuration->nx * coachella_configuration->ny * coachella_configuration->nz * sizeof(float);
	int file_count = 0;
	...
	sprintf(current_file, "%s/vp.dat", coachella_data_directory);
...
/**
 * Init function loaded and called by the UCVM library. Calls coachella_init.
 *
 * @param dir The directory in which UCVM is installed.
 * @return Success or failure.
 */
int model_init(const char *dir, const char *label) {
	return coachella_init(dir, label);
}

/**
 * Query function loaded and called by the UCVM library. Calls coachella_query.
 *
 ...
 * @return Success or fail.
 */
int model_query(coachella_point_t *points, coachella_properties_t *data, int numpoints) {
	return coachella_query(points, data, numpoints);
}

/**
 * Finalize function loaded and called by the UCVM library. Calls coachella_finalize.
 *
 * @return Success
 */
int model_finalize() {
	return coachella_finalize();
}

/**
 * Version function loaded and called by the UCVM library. Calls coachella_version.
 *
 * @param ver Version string to return.
 * @param len Maximum length of buffer.
 * @return Zero
...
int model_version(char *ver, int len) {
	return coachella_version(ver, len);
}

int (*get_model_init())(const char *, const char *) {
        return &coachella_init;
}
int (*get_model_query())(coachella_point_t *, coachella_properties_t *, int) {
         return &coachella_query;
}
int (*get_model_finalize())() {
         return &coachella_finalize;
}
int (*get_model_version())(char *, int) {
         return &coachella_version;
}
