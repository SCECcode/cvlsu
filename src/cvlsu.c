...
int cvlsu_init(const char *dir, const char *label) {
	...
	cvlsu_velocity_model = calloc(1, sizeof(cvlsu_model_t));
	...
	cvlsu_total_height_m = sqrt(pow(cvlsu_configuration->top_left_corner_lat - cvlsu_configuration->bottom_left_corner_lat, 2.0f) +
 ...
 */
int cvlsu_query(cvlsu_point_t *points, cvlsu_properties_t *data, int numpoints) {
	int i = 0;
	int load_x_coord = 0, load_y_coord = 0, load_z_coord = 0;
	double x_percent = 0, y_percent = 0, z_percent = 0;
	cvlsu_properties_t surrounding_points[8];
        double lon_e, lat_n;

	int zone = 11;
	int longlat2utm = 0;

        double delta_lon = (cvlsu_configuration->top_right_corner_lon - cvlsu_configuration->bottom_left_corner_lon)/(cvlsu_configuration->nx - 1);
        double delta_lat = (cvlsu_configuration->top_right_corner_lat - cvlsu_configuration->bottom_left_corner_lat)/(cvlsu_configuration->ny - 1);
...
//fprintf(stderr,"coord %d %d %d\n", load_x_coord, load_y_coord, load_z_coord);
		...
		x_percent =fmod((lon_e - cvlsu_configuration->bottom_left_corner_lon), delta_lon) /delta_lon;
		y_percent = fmod((lat_n - cvlsu_configuration->bottom_left_corner_lat), delta_lat)/
delta_lat;
...
//fprintf(stderr,"percent %lf %lf %lf\n", x_percent, y_percent, z_percent);
		if (load_z_coord == 0 && z_percent == 0) {
			...
			load_z_coord = 0;
                   if(cvlsu_configuration->interpolation) {

			// Get the four properties.
			cvlsu_read_properties(load_x_coord,     load_y_coord,     load_z_coord,     &(surrounding_points[0]));	// Orgin.
			cvlsu_read_properties(load_x_coord + 1, load_y_coord,     load_z_coord,     &(surrounding_points[1]));	// Orgin + 1x
			cvlsu_read_properties(load_x_coord,     load_y_coord + 1, load_z_coord,     &(surrounding_points[2]));	// Orgin + 1y
			cvlsu_read_properties(load_x_coord + 1, load_y_coord + 1, load_z_coord,     &(surrounding_points[3]));	// Orgin + x + y, forms top plane.

			cvlsu_bilinear_interpolation(x_percent, y_percent, surrounding_points, &(data[i]));
                  } else {
			cvlsu_read_properties(load_x_coord,     load_y_coord,     load_z_coord,     &(data[i]));	// Orgin.
                  }
		  ...
		  if( cvlsu_configuration->interpolation) {
			...
			cvlsu_read_properties(load_x_coord,     load_y_coord + 1, load_z_coord,     &(surrounding_points[2]));	// Orgin + 1y
			cvlsu_read_properties(load_x_coord + 1, load_y_coord + 1, load_z_coord,     &(surrounding_points[3]));	// Orgin + x + y, forms top plane.
			cvlsu_read_properties(load_x_coord,     load_y_coord,     load_z_coord - 1, &(surrounding_points[4]));	// Bottom plane origin
			cvlsu_read_properties(load_x_coord + 1, load_y_coord,     load_z_coord - 1, &(surrounding_points[5]));	// +1x
			cvlsu_read_properties(load_x_coord,     load_y_coord + 1, load_z_coord - 1, &(surrounding_points[6]));	// +1y
			cvlsu_read_properties(load_x_coord + 1, load_y_coord + 1, load_z_coord - 1, &(surrounding_points[7]));	// +x +y, forms bottom plane.

			cvlsu_trilinear_interpolation(x_percent, y_percent, z_percent, surrounding_points, &(data[i]));
                    } else {
 ...
 */
void cvlsu_trilinear_interpolation(double x_percent, double y_percent, double z_percent,
							 cvlsu_properties_t *eight_points, cvlsu_properties_t *ret_properties) {
	cvlsu_properties_t *temp_array = calloc(2, sizeof(cvlsu_properties_t));
	cvlsu_properties_t *four_points = eight_points;

	cvlsu_bilinear_interpolation(x_percent, y_percent, four_points, &temp_array[0]);

	// Now advance the pointer four "cvlsu_properties_t" spaces.
	four_points += 4;

	// Another interpolation.
	cvlsu_bilinear_interpolation(x_percent, y_percent, four_points, &temp_array[1]);

	// Now linearly interpolate between the two.
	cvlsu_linear_interpolation(z_percent, &temp_array[0], &temp_array[1], ret_properties);
...
void cvlsu_bilinear_interpolation(double x_percent, double y_percent, cvlsu_properties_t *four_points, cvlsu_properties_t *ret_properties) {
	...
	cvlsu_linear_interpolation(x_percent, &four_points[0], &four_points[1], &temp_array[0]);
	cvlsu_linear_interpolation(x_percent, &four_points[2], &four_points[3], &temp_array[1]);
	cvlsu_linear_interpolation(y_percent, &temp_array[0], &temp_array[1], ret_properties);
 ...
 */
int cvlsu_finalize() {
	if (cvlsu_velocity_model->vp) free(cvlsu_velocity_model->vp);

	free(cvlsu_configuration);
...
{
  int verlen;
  verlen = strlen(cvlsu_version_string);
  if (verlen > len - 1) {
    verlen = len - 1;
  }
  memset(ver, 0, len);
...
void print_error(char *err) {
	fprintf(stderr, "An error has occurred while executing CVLSU. The error was:\n\n");
	fprintf(stderr, "%s", err);
	fprintf(stderr, "\n\nPlease contact software@scec.org and describe both the error and a bit\n");
	fprintf(stderr, "about the computer you are running CVLSU on (Linux, Mac, etc.).\n");
}
...
int cvlsu_try_reading_model(cvlsu_model_t *model) {
	double base_malloc = cvlsu_configuration->nx * cvlsu_configuration->ny * cvlsu_configuration->nz * sizeof(float);
	int file_count = 0;
	...
	sprintf(current_file, "%s/vp.dat", cvlsu_data_directory);
...
/**
 * Init function loaded and called by the UCVM library. Calls cvlsu_init.
 *
 * @param dir The directory in which UCVM is installed.
 * @return Success or failure.
 */
int model_init(const char *dir, const char *label) {
	return cvlsu_init(dir, label);
}

/**
 * Query function loaded and called by the UCVM library. Calls cvlsu_query.
 *
 ...
 * @return Success or fail.
 */
int model_query(cvlsu_point_t *points, cvlsu_properties_t *data, int numpoints) {
	return cvlsu_query(points, data, numpoints);
}

/**
 * Finalize function loaded and called by the UCVM library. Calls cvlsu_finalize.
 *
 * @return Success
 */
int model_finalize() {
	return cvlsu_finalize();
}

/**
 * Version function loaded and called by the UCVM library. Calls cvlsu_version.
 *
 * @param ver Version string to return.
 * @param len Maximum length of buffer.
 * @return Zero
...
int (*get_model_query())(cvlsu_point_t *, cvlsu_properties_t *, int) {
         return &cvlsu_query;
}
